...
export default function SuccessPage() {
  ...
  const orderId = searchParams.get('orderId') || 'ECO' + Math.floor(100000 + Math.random() * 900000)
    ...
    <div className="min-h-screen bg-cream-50 relative overflow-hidden flex flex-col">
      ...
      <header className="border-b border-cream-200 bg-cream-50/90 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-4 py-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-forest-700 flex items-center justify-center text-cream-100 text-sm font-display font-bold">
              E
            </div>
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <div className={`max-w-md w-full text-center transition-all duration-700 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-forest-500 font-body text-base mb-6">
            Thank you for choosing a greener planet.
          </p>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-cream-200 text-left space-y-4 mb-6">
            ...
            <div className="p-3 bg-forest-50 rounded-xl border border-forest-100">
              <p className="text-xs text-forest-600 font-body">
                📧 A confirmation email will be sent with your tracking details.
              </p>
            </div>
          </div>
          ...
          <div className="bg-forest-950 rounded-2xl p-5 text-left mb-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { icon: '🌳', value: '1', label: 'Tree Planted' },
                { icon: '♻️', value: '2.3kg', label: 'Plastic Saved' },
                { icon: '💨', value: '0.4kg', label: 'CO₂ Offset' },
              ].map(stat => (
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link
              href="/"
              className="btn-eco flex-1 py-3.5 bg-forest-700 hover:bg-forest-600 text-white rounded-xl font-body font-semibold text-sm transition-all shadow-md shadow-forest-200 flex items-center justify-center gap-2"
            >
              Continue Shopping
            </Link>
            <button
              onClick={() => window.print()}
              className="flex-1 py-3.5 border border-cream-300 hover:border-forest-300 text-forest-700 rounded-xl font-body font-semibold text-sm transition-colors flex items-center justify-center gap-2"
            >
